...
# Comment out lines 52 and 65-68 as needed if you don't want to exclude serial samples or duplicate healthy libraries.
...
rule all:
    input:
        ...
        expand(frag_beds + "/{library}_short_weights_{cutpoint}.bed", library = ALL_LIBRARIES, cutpoint = cutpoints),
        expand(frag_beds + "/{library}_long_weights_{cutpoint}.bed", library = ALL_LIBRARIES, cutpoint = cutpoints),
        expand(frag_counts + "/{library}_count_total.tsv", library = ALL_LIBRARIES),
        expand(frag_counts + "/{library}_count_short_{cutpoint}.tsv", library = ALL_LIBRARIES, cutpoint = cutpoints),
        expand(frag_counts + "/{library}_count_long_{cutpoint}.tsv", library = ALL_LIBRARIES, cutpoint = cutpoints),
        frag + "/frag_counts.tsv",
        expand(frag + "/frag_counts_{cutpoint}.tsv", cutpoint = cutpoints),
        expand(frag + "/ratios_{cutpoint}.tsv", cutpoint = cutpoints),
        frag + "/arm_z.tsv"
...
rule frag_weighting_and_size_split:
    ...
    output: 
        ...
        short = [frag_beds + "/{library}_short_weights_" + str(cutpoint) + ".bed" for cutpoint in cutpoints],
        long = [frag_beds + "/{library}_long_weights_" + str(cutpoint) + ".bed" for cutpoint in cutpoints]
    params:
...
rule frag_total_window_count:
    benchmark: benchdir + "/{library}_frag_total_window_count.benchmark.txt",
    input: 
        weights_bed = frag_beds + "/{library}_weights.bed",
        keep_bed = refdir + "/keep_5mb.bed",
    log: logdir + "/{library}_frag_total_window_count.log",
    output: frag_counts + "/{library}_count_total.tsv",
    params:
...
rule frag_short_and_long_window_count:
    benchmark: benchdir + "/{library}_frag_window_count_{cutpoint}.benchmark.txt",
    input:
        short = frag_beds + "/{library}_short_weights_{cutpoint}.bed",
        long = frag_beds + "/{library}_long_weights_{cutpoint}.bed",
        keep_bed = refdir + "/keep_5mb.bed",
    log: logdir + "/{library}_frag_window_count_{cutpoint}.log",
    output:
        short = frag_counts + "/{library}_count_short_{cutpoint}.tsv",
        long = frag_counts + "/{library}_count_long_{cutpoint}.tsv",
    params:
        script = scriptdir + "/frag_window_count.R",
        threads = threads,
    shell:
...
rule total_count_merge:
    benchmark: benchdir + "/total_count_merge.benchmark.txt",
    input: expand(frag_counts + "/{library}_count_total.tsv", library = ALL_LIBRARIES),
    log: logdir + "/total_count_merge.log",
    output: frag + "/frag_counts.tsv",
    params:
...
rule short_and_long_count_merge:
    benchmark: benchdir + "/short_and_long_count_merge.benchmark.txt"
    input:
        short = expand(frag_counts + "/{library}_count_short_{cutpoint}.tsv", library = ALL_LIBRARIES, cutpoint = cutpoints),
        long = expand(frag_counts + "/{library}_count_long_{cutpoint}.tsv", library = ALL_LIBRARIES, cutpoint = cutpoints)
    log: logdir + "/short_and_long_count_merge.log"
    output:
        expand(frag + "/frag_counts_{cutpoint}.tsv", cutpoint = cutpoints)
    params:
...
rule make_ratios:
    ...
    input: frag + "/frag_counts_{cutpoint}.tsv",
    log: logdir + "/make_ratios_{cutpoint}.log",
    output: frag + "/ratios_{cutpoint}.tsv",
    params:
