...
frag_length_low = 30
frag_length_high = 750
cutpoints = [140, 143, 150, 158]
...
rule all:
    input:
        ...
        # expand(beds_dir + "/{library}_long_weights_{cutpoint}.bed", library = ALL_LIBRARIES, cutpoint = cutpoints),
        # expand(counts_dir + "/{library}_count_total.tsv", library = ALL_LIBRARIES),
        # expand(counts_dir + "/{library}_count_short_{cutpoint}.tsv", library = ALL_LIBRARIES, cutpoint = cutpoints),
...
#         short = [beds_dir + "/{library}_short_weights_" + str(cutpoint) + ".bed" for cutpoint in cutpoints],
...
#     input:
#         short = beds_dir + "/{library}_short_weights_{cutpoint}.bed",
#         long = beds_dir + "/{library}_long_weights_{cutpoint}.bed",
#         keep_bed = refdir + "/keep_5mb.bed",
#     log: logdir + "/{library}_frag_window_count_{cutpoint}.log",
#     output:
#         short = counts_dir + "/{library}_count_short_{cutpoint}.tsv",
...
#     input: expand(counts_dir + "/{library}_count_total.tsv", library = ALL_LIBRARIES),
...
rule short_and_long_count_merge:
    ...
    input:
        short = expand(counts_dir + "/{library}_count_short_{cutpoint}.tsv", library = ALL_LIBRARIES, cutpoint = cutpoints),
        long = expand(counts_dir + "/{library}_count_long_{cutpoint}.tsv", library = ALL_LIBRARIES, cutpoint = cutpoints)
